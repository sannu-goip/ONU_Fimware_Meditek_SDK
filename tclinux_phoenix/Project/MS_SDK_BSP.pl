...
my @cutFolder;
my $dir = getcwd;
my $datePath="$dir/linux_trunk_log/bsp/$DateNoSymbol";
...
$retValue = &system_cmd("(echo $profile_selest && echo y) | osbnq64 fakeroot make -f Project/MakeFile_Release_BSP CUSTOM=CT RELEASEBSP=y MSDK=1 confirm_infor release_dir_delete release_all_check");
$curTime=strftime "%Y-%m-%d %H:%M:%S",localtime;
if($retValue eq 0){
		print "release_all_check success\r\n";
		system("echo release_all_check success:$curTime >> $compileRet");
}else {
		print "release_all_check fail\r\n";
...
my $buildLog_releasTar="$dir/linux_trunk_log/bsp/$DateNoSymbol/releasTar.log";
...
$retValue = &system_cmd("osbnq64 fakeroot make -f Project/MakeFile_Release_BSP CUSTOM=CT RELEASEBSP=y MSDK=1 release_profile_bsp release_makefile_bsp release_config_bsp release_doc release_swept_bsp release_tar_bsp",$buildLog_releasTar);
$curTime=strftime "%Y-%m-%d %H:%M:%S",localtime;
if($retValue eq 0){
...
sub msdk_releasebsp_flow(){
	...
	my $profile;
	my $buildBase;	
	my $buildCom="osbnq64 fakeroot make -f Project/MakeFile_Release_BSP CUSTOM=CT PROFILE=$buildProfile MSDK=1 release_backup_bsp BAK_PATH=$buildBaseSrc";
	my $value_ret;
	my $startTime=strftime "%Y-%m-%d %H:%M:%S",localtime;
	my $str = "7523";
	...
	if($buildProfile =~ /$str/)
	{
		$buildCom="osbnq64 fakeroot make -f Project/MakeFile_Release_BSP CUSTOM=CT PROFILE=$buildProfile MSDK=1 release_backup_bsp BAK_PATH=$buildBaseSrc";
	}
	...
	if(-e "$codePath/BSP_$countNum"){
		&system_cmd("rm -rf $codePath/BSP_$countNum");
		&system_cmd("mkdir $codePath/BSP_$countNum");
		&system_cmd("cp -rf $codePath/tclinux_phoenix $codePath/BSP_$countNum");
	...
	if($value eq 0){
		if(! -e "$buildBaseSrc/release_bsp"){ 
			print "$buildBaseSrc/release_bsp is not exist\r\n";
			system("pwd");
			&system_cmd("mkdir $buildBaseSrc/release_bsp");	
		}
		system_cmd("cp -rf $buildBase/release_bsp/* $buildBaseSrc/release_bsp/");
