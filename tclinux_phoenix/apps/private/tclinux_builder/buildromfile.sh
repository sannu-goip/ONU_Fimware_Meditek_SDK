...
echo "../../../install_bsp/tools/trx/trx -d $size_f -f romfile.cfg -o ctromfile.cfg"
echo `../../../install_bsp/tools/trx/trx -d $size_f -f romfile.cfg -o ctromfile.cfg`
echo "cp ctromfile.cfg $FS_GEN_DIR/userfs"
cp ctromfile.cfg $FS_GEN_DIR/userfs

folder=$FS_GEN_DIR/userfs
for file in ${folder}/*; do
	name="${file##*/}"
	if [ "$name" != "romfile.cfg" ] && [ "$name" != "romfile_epon.cfg" ] && [ "${name:0:7}" = "romfile" ] && [ "${name/.cfg/}" != "$name" ] && [ "${name/_f.cfg/}" = "$name" ] ;then
		cp $file ./
		size_f=$(stat -c%s "$name")
		echo "../../../install_bsp/tools/trx/trx -d $size_f -f $name -o ct$name"
		echo `../../../install_bsp/tools/trx/trx -d $size_f -f $name -o ct$name`
