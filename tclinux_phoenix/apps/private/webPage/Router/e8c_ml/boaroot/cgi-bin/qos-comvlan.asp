...
<td width="50" valign="bottom"><a href="/cgi-bin/login.asp" id=logOut name=logOut target="_top" onClick="DoLogout()">
<span class="logout">
...
{
alert('<%tcWebApi_get("String_Entry","NoWANConnectText","s")%>');
return false;
}
...
<tr align='center'>
<td class="table_title"><%tcWebApi_get("String_Entry","InterfaceText","s")%></td>
<td class="table_title">VLAN</td>
</tr>
<tr align="middle">
<td>
					<input type="hidden" id="Changed_Flag" name="Changed_Flag" value="0">
                    <input type="hidden" id="VLanActive" name="VLanActive" value="<% tcWebApi_get("QoS_Entry","Active","s") %>">
                    <input type="hidden" id="curIndex" name="curIndex" value="<% tcWebApi_get("WebCurSet_Entry","qos_id","s") %>">
                    <input type="hidden" id="curIndexshow" name="curIndexshow" value="<% tcWebApi_get("QoS_Entry","EntryIndex","s") %>">
                     <input type="hidden" id="vlanRuleFlag" name="vlanRuleFlag" value="vlanidRule">
					  <input type="hidden" id="pvcindex" name="pvcindex" value=""> 
...
function	BtnSave()
{
	...
	if(parseInt(Form.nameCOM_VLAN.value) <0 ||  parseInt(Form.nameCOM_VLAN.value) >4094)
	{
		...
		{
			alert("<%tcWebApi_get("String_Entry","IncorrectVLANValueText","s")%>");
			return;
		}
	}
...
<br>
<input type='button' onClick='btnRtturnQoS()' id=btnRtturnQoS name=btnRtturnQoS value='<%tcWebApi_get("String_Entry","BackText","s")%>'>
</p>
</td>
<td width="10">&nbsp;</td>
</tr>
...
<p align="center"><img src="<%tcWebApi_get("String_Entry","IMGOKText","s")%>" width="80" height="23" border="0" id="btnOK" onClick=BtnSave()>&nbsp;&nbsp;<img src="<%tcWebApi_get("String_Entry","IMGCancelText","s")%>" width="80" height="23" border="0" id="btnCancel" onclick="RefreshPage()"> </td>
