					...
					int type)
{
	...
	if (pfind == NULL) {
		...
		/* with dir */
		/* save for change */
		strncpy(dirbuf, command,sizeof(dirbuf)-1);
		/* set dir only */
...
static int find_match(char *matchBuf, int *len_with_quotes)
{
	...
	int command_mode;
	int c, c2;
	static int int_buf[BUFSIZ + 1];
	static int pos_buf[BUFSIZ + 1];
	...
	if (! *lastWasTab) {
		...
		*lastWasTab = TRUE;             /* flop trigger */
		...
		if ((len_found - strlen(matchBuf) + len) < BUFSIZ) {
			...
			/* save   tail line        */
			strncpy(matchBuf, command_ps + cursor,sizeof(matchBuf)-1);
			/* add    match            */
