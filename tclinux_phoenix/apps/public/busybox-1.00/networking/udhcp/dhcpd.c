...
/* define */
#if defined(TCSUPPORT_DYNAMIC_VLAN)
#define DYVLAN_TMP_FILE "/tmp/var/dyVLAN"
#endif
/* globals */
...
struct server_config_t option60_camera_config;//option 60  camera pool dhcp server
struct server_config_t option60_hgw_config;//option 60 hgw pool dhcp server
struct server_config_t option60_phone_config;//option 60 phone pool dhcp server
struct server_config_t option60_stb_config;////option 60 stb pool dhcp server
#endif
struct Option60_Param_s gOption60_Param;
#if defined(TCSUPPORT_CT_E8GUI)
struct Option60_Param_s gOption60_camera_Param;
struct Option60_Param_s gOption60_hgw_Param;
struct Option60_Param_s gOption60_phone_Param;
struct Option60_Param_s gOption60_stb_Param;
#endif
...
#if defined(TCSUPPORT_DYNAMIC_VLAN)
static int readDyVlanFile(char *Strfile)
{
        FILE *fp=NULL;
        fp = fopen(DYVLAN_TMP_FILE, "r");
        if(fp == NULL){
                return -1;
        }
        fgets(Strfile , 256 , fp);
        fclose(fp);
        return 0;
}

static int DyVlanDrop(struct dhcpMessage *packetPtr)
{
        int i = 0;
        int enableFlag = 0;
        int uselessPvcFlag = 0;
        char Strfile[256] = "";
        char strOui[5][10];
        char mac_tmp[20] = "";
        
        memset(Strfile , 0 , sizeof(Strfile));            
        memset(strOui , 0 , sizeof(strOui));    

        // get oui from /tmp/var/dyVLAN
        readDyVlanFile(Strfile);
        sscanf(Strfile , "%d %s %d %s %d %s %d %s %d %s %d" ,
                &enableFlag , strOui[0] , &uselessPvcFlag ,strOui[1] , &uselessPvcFlag ,strOui[2] ,&uselessPvcFlag ,strOui[3] ,&uselessPvcFlag ,strOui[4], &uselessPvcFlag);
        
        // get oui from dhcp packet
        sprintf(mac_tmp , "%02x%02x%02x" , 
                packetPtr->chaddr[0] , packetPtr->chaddr[1] ,packetPtr->chaddr[2]);

        for(i = 0; i < 5; i++)
        {
                // when the oui is the same as the oui which send dhcp 
                // don't send offer to client. The client would get offer from remote server
                if( (1 == enableFlag) && ( 0 == strcasecmp(mac_tmp , strOui[i]) ) )
                        return 1;
        }  
        return 0;
}
...
int udhcpd_parser_option60(uint8_t *vendorID){
	...
	if((vendorID[0] == enterprise_num[0]) && (vendorID[1] == enterprise_num[1])){
		...
		if (gOption60_stb_Param.dhcp_opt60_vid && strstr((char *)& vendorID[2], gOption60_stb_Param.dhcp_opt60_vid) != NULL)
		{
			memcpy(&server_config, &option60_stb_config, sizeof(struct server_config_t));
			gOption60IsMatched = 2;
			tcdbg_printf("\nget device is STB");
		}
		if(!gOption60IsMatched){
			if (gOption60_phone_Param.dhcp_opt60_vid && strstr((char *)& vendorID[2], gOption60_phone_Param.dhcp_opt60_vid) != NULL)
			{
				memcpy(&server_config, &option60_phone_config, sizeof(struct server_config_t));
				gOption60IsMatched = 3;
				tcdbg_printf("\nget device is phone");
			}
		}
		if(!gOption60IsMatched){
			if (gOption60_camera_Param.dhcp_opt60_vid && strstr((char *)& vendorID[2], gOption60_camera_Param.dhcp_opt60_vid) != NULL)
			{
				memcpy(&server_config, &option60_camera_config, sizeof(struct server_config_t));
				gOption60IsMatched = 4;
				tcdbg_printf("\nget device is camera");
			}
		}
		if(!gOption60IsMatched){
			if (gOption60_hgw_Param.dhcp_opt60_vid && strstr((char *)& vendorID[2], gOption60_hgw_Param.dhcp_opt60_vid) != NULL)
			{
				memcpy(&server_config, &option60_hgw_config, sizeof(struct server_config_t));
	...
	server_config.lease = option60_config.lease;
	server_config.max_leases = option60_config.max_leases;
	server_config.ifindex = option60_config.ifindex;
	server_config.server = option60_config.server;
	return 0;
}
#endif
#if defined(TCSUPPORT_CT_ACCESSLIMIT) && !defined(TCSUPPORT_VLAN_ACCESS_TRUNK)
...
#endif
{
	...
	if(config_file_exist) {
		...
		memset(&gOption60_Param, 0, sizeof(struct Option60_Param_s));
		read_config(DHCPD_OPTION_CAMERA_CONF_FILE);
		memcpy(&option60_camera_config, &option60_config, sizeof(struct server_config_t));
		memcpy(&gOption60_camera_Param, &gOption60_Param, sizeof(struct Option60_Param_s));

		memset(&option60_config, 0, sizeof(struct server_config_t));
		memset(&gOption60_Param, 0, sizeof(struct Option60_Param_s));
		read_config(DHCPD_OPTION_HGW_CONF_FILE);
		memcpy(&option60_hgw_config, &option60_config, sizeof(struct server_config_t));
		memcpy(&gOption60_hgw_Param, &gOption60_Param, sizeof(struct Option60_Param_s));

		memset(&option60_config, 0, sizeof(struct server_config_t));
		memset(&gOption60_Param, 0, sizeof(struct Option60_Param_s));
		read_config(DHCPD_OPTION_PHONE_CONF_FILE);
		memcpy(&option60_phone_config, &option60_config, sizeof(struct server_config_t));
		memcpy(&gOption60_phone_Param, &gOption60_Param, sizeof(struct Option60_Param_s));

		memset(&option60_config, 0, sizeof(struct server_config_t));
		memset(&gOption60_Param, 0, sizeof(struct Option60_Param_s));
		read_config(DHCPD_OPTION_STB_CONF_FILE);
		memcpy(&option60_stb_config, &option60_config, sizeof(struct server_config_t));
		memcpy(&gOption60_stb_Param, &gOption60_Param, sizeof(struct Option60_Param_s));
#endif
...
/*account the total max leases of all pools*/
	...
	if(gOption60IsMatched) {		
...
#if defined(TCSUPPORT_CT_IPTV_NODE) && !defined(TCSUPPORT_VLAN_ACCESS_TRUNK)
		...
		case DHCPRELEASE:
			...
			snprintf(data, sizeof(data), "del;%02X:%02X:%02X:%02X:%02X:%02X", packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5]);
			res = ecnt_event_send(ECNT_EVENT_LANHOST, ECNT_EVENT_LANHOST_DHCP_UPDATE, data, strlen(data) + 1);
#endif
			break;
		case DHCPINFORM:
