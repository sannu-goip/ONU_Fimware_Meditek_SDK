...
#if defined(TCSUPPORT_CT)
#ifdef TCSUPPORT_SYSLOG
		if(!strcmp(client_server, "udhcpd"))
			openlog("TCSysLog udhcpd", LOG_PID | LOG_CONS, LOG_LOCAL0);
		else if(!strcmp(client_server, "udhcpc"))
			openlog("TCSysLog udhcpc", LOG_PID | LOG_CONS, LOG_LOCAL0);
		else
