...
static char **fill_envp(struct dhcpMessage *packet)
{
	...
	if ( client_config.ipoe_if )
	{
		memcpy(&netmask, &subnet, 4);
		dstnet = packet->yiaddr & netmask;
		envp[j] = xmalloc(sizeof("dstnet=255.255.255.255"));
		sprintip(envp[j++], "dstnet=", (uint8_t *) &dstnet);
	}
#endif
	if (packet->siaddr) {
