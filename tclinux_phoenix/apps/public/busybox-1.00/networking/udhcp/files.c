...
int read_config(const char *file)
{
...
#ifdef TCSUPPORT_FON_V2
		|| (strcmp(file, FON_DHCPD_CONF_FILE) == 0)
#endif
	)
		pKeywords = keywords;
#ifdef DHCP_PROFILE
	...
	else if((!strcmp(file, DHCPD_OPTION_STB_CONF_FILE)) || (!strcmp(file, DHCPD_OPTION_PHONE_CONF_FILE)) || (!strcmp(file, DHCPD_OPTION_CAMERA_CONF_FILE))
		 || (!strcmp(file, DHCPD_OPTION_HGW_CONF_FILE)))
		pKeywords = option_keywords;
#endif
...
void write_leases(void)
{
	...
	if (!(fp = fopen(dhcp_config.lease_file, "w"))) {
		LOG(LOG_ERR, "Unable to open %s for writing", dhcp_config.lease_file);
		return;
	}
		...
		if (leases[i].yiaddr != 0) {
			...
			if (dhcp_config.remaining) {
				if (lease_expired(&(leases[i])))
					leases[i].expires = 0;
				else leases[i].expires -= curr.tv_sec;
	...
	if (dhcp_config.notify_file) {
		sprintf(buf, "%s %s", dhcp_config.notify_file, dhcp_config.lease_file);
		system(buf);
	}
#else
...
void write_leases_enhance(struct dhcpOfferedAddr *lease)
{
	...
	for (i = 0; i < gTotalMaxLeases; i++) {
		...
		if (leases[i].yiaddr != 0) {
			...
			sprintf(buf, "%s %s %ld %s %s %s\n", mac_addr, ip_addr, expires, leases[i].hostname,leases[i].devicetype,leases[i].dhcpoption60);
			#else
	...
	if(leaseNumber != currentLeaseNumer){
		...
		char data[32] = {0};
		int res = 0;
		res = ecnt_event_send(ECNT_EVENT_LANHOST, ECNT_EVENT_LANHOST_DHCP_REINIT, data, 0);
#endif
	}
}
...
void read_leases(const char *file)
{
	...
	while (i < gTotalMaxLeases && (fread(&lease, sizeof lease, 1, fp) == 1)) {
