...
struct dhcpOfferedAddr *add_lease_option(uint8_t *chaddr, uint32_t yiaddr, unsigned long lease, char *hostname, char *devicetype, char *optionvalue)
{
	...
	if (oldest) {
		...
		if(devicetype){
			strncpy(oldest->devicetype, devicetype, HOST_DEVICETYPE_LEN);
		}
		if(optionvalue){
			strncpy(oldest->dhcpoption60, optionvalue, DHCP_OPTION_LEN);
		}
	}
...
struct dhcpOfferedAddr *add_lease(uint8_t *chaddr, uint32_t yiaddr, unsigned long lease, char *hostname)
{
	...
	if (oldest) {
		...
		//oldest->expires = time(0) + lease;
		if(hostname){			
			strncpy(oldest->hostname, hostname, HOST_NAME_LEN - 1);
		}		
	}
...
static int check_ip(uint32_t addr)
{
	...
	if (arpping(addr, server_config.server, server_config.arp, server_config.interface) == 0) {
		...
		LOG(LOG_INFO, "client IP belongs to someone, reserving it for %ld seconds",
