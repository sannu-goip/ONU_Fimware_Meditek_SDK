...
unsigned char ipoe_dhcp125_option[34] = {0};
int serverid_index = -1;
...
static void perform_release(void)
{
	...
	if (state == BOUND || state == RENEWING || state == REBINDING || state == REQUESTING || state == RENEW_REQUESTED) {
		...
		LOG(LOG_INFO, "Unicasting a release of requset IP to server IP");
#else
...
static uint8_t send_release_packet(char *wanIF){
	char wan_node[32] = {0};
	char wan_if_val[20] = {0};
	unsigned long requested_ip_last = 0;
	unsigned long server_addr_last = 0;
	char cmd[256] = {0};
	char last_ip[20] = {0};
	char last_gw[20] = {0};
	char last_mask[20] = {0};
	char last_gw_mac[64] = {0};
	uint8_t raw_mac[64] = {0};
	char first_discover[8] = {0};
	int mac_tmp[6] = {0};
	char buf[256] ={0};
	char etherSta[32] ={0};
	int i = 0;
	...
	memset(buf, 0, sizeof(buf));
	tcapi_get("Wan_Common", "dhcp_release", buf);	
	if(strcmp(buf, "1") != 0){
		return 1;
	}
	
	memset(buf, 0, sizeof(buf));
	tcapi_get("XPON_Common", "trafficStatus", buf); 
...
#else
	memset(etherSta, 0, sizeof(etherSta));
	tcapi_get("WanInfo_Common", "EthernetState", etherSta); 
	if(strcmp(buf, "up") != 0 && strcmp(etherSta, "up") != 0 ){
		return 1;
	}
#endif
	if(wanIF == NULL){
	...
	if(0 != tcapi_get("WanInfo_Common", "first_discover", first_discover) || strcmp(first_discover,"1") == 0){
		tcdbg_printf("[%s:%d]first_discover=1, send release before discover\n",__FUNCTION__,__LINE__);	
		
		sscanf(wanIF, "nas%d_%d", &i, &j);	
		memset(wan_node,0,sizeof(wan_node));
		snprintf(wan_node,sizeof(wan_node), "Wan_PVC%d_Entry%d", i, j);
			

		/*get ip*/
		memset(&last_ip, 0, sizeof(last_ip));
		tcapi_get(wan_node, "IP_last", last_ip);	

		/*get gw*/
		memset(&last_gw, 0, sizeof(last_gw));
		tcapi_get(wan_node, "GW_last", last_gw);	
		if(strlen(last_ip) != 0 && strlen(last_gw) != 0){			
			server_addr_last = inet_addr(last_gw);						
			requested_ip_last = inet_addr(last_ip); 

			/*get mac */
			memset(&cmd, 0, sizeof(cmd));
			memset(&last_gw_mac, 0, sizeof(last_gw_mac));
			tcapi_get(wan_node, "GW_mac_last", last_gw_mac);	
			if(strlen(last_gw_mac) > 0){
				sscanf(last_gw_mac, "%02x:%02x:%02x:%02x:%02x:%02x",
					&mac_tmp[0],&mac_tmp[1],&mac_tmp[2],&mac_tmp[3],&mac_tmp[4],&mac_tmp[5]);
			
				raw_mac[0] = (uint8_t)mac_tmp[0];
				raw_mac[1] = (uint8_t)mac_tmp[1];
				raw_mac[2] = (uint8_t)mac_tmp[2];
				raw_mac[3] = (uint8_t)mac_tmp[3];
				raw_mac[4] = (uint8_t)mac_tmp[4];
				raw_mac[5] = (uint8_t)mac_tmp[5];
		
			}		

			send_release_raw(server_addr_last, requested_ip_last, raw_mac); /* unicast */
			tcapi_set("WanInfo_Common", "first_discover", "0"); 	
...
#if defined(TCSUPPORT_CT_DHCPC_OPTION60)
int make_option60_portmapping_msg(unsigned char *msg, int msg_size)
{
    int i = 0, n = 0;
    char nodeName[32] = {0};
    char activeVal[32] = {0};
    char protocalVal[32] = {0};
    char intPortVal[32] = {0};
    unsigned short protocal = 0;
    ...
    for(i = 0;i < 10;i ++)
    {
        ...
        memset(protocalVal, 0, sizeof(protocalVal));
        memset(intPortVal, 0, sizeof(intPortVal));
        
        snprintf(nodeName, sizeof(nodeName), "VirServer_Entry%d", i);
        tcapi_get(nodeName, "Active", activeVal);
        tcapi_get(nodeName, "Protocal", protocalVal);
        tcapi_get(nodeName, "INTPORT", intPortVal);
        
        if(0 != strcmp("Yes", activeVal) || '\0' == protocalVal[0] || '\0' == intPortVal[0])
            continue;
        
        if(0 == strcmp("UDP", protocalVal))
        {
            protocal= 0x00;
        }
        else if(0 == strcmp("TCP", protocalVal))
        {
            protocal = 0x01;
        }
        else if(0 == strcmp("TCP/UDP", protocalVal))
        {
            protocal = 0x02;
        }
        intPort = atoi(intPortVal);
        
        /*Use 4 bytes to represent one portmapping rule*/
        /*n means the number of the portmapping rules*/
        /*The first two bytes mean protocal(0: UDP, 1: TCP, 2: TCP/UDP)*/
        /*The last two bytes mean port number*/
        protocal = htons(protocal);
        intPort = htons(intPort);
        memcpy(msg + (4 * n), &protocal, 2);
        memcpy(msg + (4 * n + 2), &intPort, 2);
...
void hexToString(char *dstStr, int strSize, unsigned char *hexArray, int arrSize)
{
    int i = 0;
    char tmp[8] = {0};

    memset(dstStr, 0, strSize);
    for(i = 0;i < arrSize;i++)
    {
        if(2 * i >= strSize || i >= arrSize)
        {
            break;
        }
        snprintf(tmp, sizeof(tmp), "%02X", hexArray[i]);
        strncat(dstStr, tmp, sizeof(dstStr) - 1);
    }
...
static uint8_t make_option60(char *ifname, struct client_config_t *conf)
{
	...
	unsigned char *option60Str = xmalloc(256);
#if defined(TCSUPPORT_CT_JOYME4) && defined(TCSUPPORT_CT_VRWAN)
	char serviceList[32] = {0};
#endif

	memset(option60Str, 0, 256);
#if defined(TCSUPPORT_CT_IPOE_EMULATOR)
	...
	for(i=1; i<5; i++){
		...
		if(!strcmp(tmp, "Yes")){
			...
			if ( 0 != tcapi_get(nodeName, attrName, tmp) || tmp[0] == '\0')
				strcpy(tmp, "34");
			type = atoi(tmp);
			...
			if( !strcmp(serviceList, "SPECIAL_SERVICE_VR") && 0 == tmp[0] )
				valueMode = 2;
			else
#endif
...
#if defined(TCSUPPORT_CT_JOYME4)
						len = make_option60_portmapping_msg((unsigned char*)tmp, sizeof(tmp));
#endif
					}
					break;
#if defined(TCSUPPORT_CT_JOYME4)
				case 32:/*iptv authentication info*/
					if(valueMode == 0){
						snprintf(attrName, sizeof(attrName), "Option60Value%d", i);
						tcapi_get(nodeName, attrName, tmp);
					}else{
						snprintf(tmp, sizeof(tmp), "CTCDHCP0001");
					}
					break;
#if defined(TCSUPPORT_CT_VRWAN)
				case 35:/*cloudvr authentication info*/
					if(valueMode == 0){
						snprintf(attrName, sizeof(attrName), "Option60Value%d", i);
						tcapi_get(nodeName, attrName, tmp);
					}else{
						len = make_option60_voip_msg((unsigned char*)tmp, sizeof(tmp), 5);
					}
					type = 35;
					break;
#endif
...
#else
						/* valueMode = 2, F.40 */
					len = make_option60_voip_msg((unsigned char*)tmp, sizeof(tmp), 2);
#endif
					break;
				default:
					break;
...
#if defined(TCSUPPORT_CT_JOYME4)
					hexToString(hexString, sizeof(hexString), tmp, len);
					snprintf(attrName, sizeof(attrName), "Option60RelValue%d", i);
					tcapi_set(nodeName, attrName, hexString);
#endif
				}else{
					option60Str[idx+1] = strlen(tmp);/*field length*/
#if defined(TCSUPPORT_CT_JOYME4)
					snprintf(attrName, sizeof(attrName), "Option60RelValue%d", i);
					tcapi_set(nodeName, attrName, tmp);
#endif
				}
#if defined(TCSUPPORT_CT_JOYME4)
				if(type == 36)
...
static uint8_t get_option125_from_ITMS(char *ifname, struct client_config_t *conf){
	...
	for(i=1; i<5; i++){
		...
		if(!strcmp(tmp, "Yes")){
			...
			if ( 0 != tcapi_get(nodeName,attrName,tmp) )
				strcpy(tmp, "1");
			strncpy(conf->option125_info[i-1].subopt_data, tmp, sizeof(conf->option125_info[i-1].subopt_data) - 1);
...
#if defined(TCSUPPORT_CT_JOYME4)
			/*need to encode Server ID by sharedkey*/
			memset(tmp,0,sizeof(tmp));
			sprintf(attrName,"Option125ServerID%d",i);
			tcapi_get(nodeName,attrName,tmp);
			strcpy(conf->option125_info[i-1].server_id, tmp);
			
			/*Shared key*/
			memset(tmp,0,sizeof(tmp));
			sprintf(attrName,"Option125SharedKey%d",i);
			tcapi_get(nodeName,attrName,tmp);
			strcpy(conf->option125_info[i-1].shared_key, tmp);
#endif
		}else{
...
#if defined(TCSUPPORT_CT_JOYME4)
				else if(conf->option125_info[i].type == 3)
				{
					/*parse option125 message*/ 				
					msg = option125ptr + 5 + 2;/*jump 5 to subcode, jump 2 to subopt-data*/

					check = ServerID_check(msg,conf->option125_info[i].shared_key,conf->option125_info[i].server_id);
					if(check){
						msg = option125ptr - 2;
						/*auth success,cpy option125 to request packet*/
						memcpy(ipoe_dhcp125_option,msg,34);
						
						ServerID_generate(ipoe_dhcp125_option+9,conf->option125_info[i].shared_key,conf->option125_info[i].server_id);
						serverid_index = i;
						#if 0
						tcdbg_printf("\n%s[%d] ipoe_dhcp125_option = \n",__FUNCTION__,__LINE__);
						for(i = 0; i< 34; i++){
							tcdbg_printf("0x%.2X ", *(ipoe_dhcp125_option+i));
						}
						tcdbg_printf("\n");
						#endif
					}else{
						DEBUG(LOG_ERR, "ServerID is wrong in message");
						return 0;
					}
				}
#endif
...
#endif
{
...
#if defined(TCSUPPORT_CT_JOYME4)
	uint8_t *msg = NULL;
	unsigned char Ks[16] = {0};/*get from ITMS, 128 bit*/
	int check = 0;
#endif
	...
	for (;;) {
		...
		if (retval == 0) {
			...
			switch (state) {
			case INIT_SELECTING:
				send_release_packet(client_config.interface);
...
#endif
#if defined(TCSUPPORT_CT_DHCPC_OPTION60)
						if(client_config.dhcp60_option)
						{
							free(client_config.dhcp60_option);
							client_config.dhcp60_option = NULL;
							client_config.dhcp60 = 0;
						}
			...
			if (listen_mode == LISTEN_KERNEL)
				...
				tcapi_get("Wan_Common", "dhcp_release", buf);	
				if(strcmp(buf, "1") == 0){
					len = get_raw_packet(&packet, fd, last_gw_mac, 1);
					
					sscanf(client_config.interface, "nas%d_%d", &i, &j);	
					memset(wan_node,0,sizeof(wan_node));
					snprintf(wan_node, sizeof(wan_node), "Wan_PVC%d_Entry%d", i, j);
					tcapi_set(wan_node, "GW_mac_last", last_gw_mac);	
				}else{
					len = get_raw_packet(&packet, fd, last_gw_mac, 0);
				}
			...
			case REBINDING:
				...
				if (*message == DHCPACK) {
					...
					if(serverid_index >= 0){
						...
						{
							msg = temp + 5 + 2;/*jump 5 to subcode, jump 2 to subopt-data*/
							
							check = ServerID_check(msg,client_config.option125_info[serverid_index].shared_key,client_config.option125_info[serverid_index].server_id);
					...
					if(state == REBINDING)
					{
						if(0 != memcmp(packet.chaddr,client_config.arp,6))
						{
							continue;
						}
						if(sid != server_addr) 
						{
							if(tcapi_get("Dhcpd_Common", "BindingSidCheck", BindingSidCheckFlag)){
								BindingSidCheckFlag[0] = '1';
							}
							if(BindingSidCheckFlag[0] != '0'){
								continue;
							}
					...
					else /*state == RENEWING || state == REQUESTING || state == RENEW_REQUEST*/
					{
						if((sid != server_addr) || (0 != memcmp(packet.chaddr,client_config.arp,6))){
							continue;
						}
					}
#endif
...
#if defined (TCSUPPORT_NP_CTC)
					LOG(LOG_INFO, "Lease of IP obtained, lease time %ld",
											 lease);
#else
