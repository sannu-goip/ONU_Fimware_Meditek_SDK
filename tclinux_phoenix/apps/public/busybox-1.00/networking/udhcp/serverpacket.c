...
#endif
#if defined(TCSUPPORT_CT_DHCPC_OPTION125) && !defined(TCSUPPORT_VLAN_ACCESS_TRUNK)	
#if defined(TCSUPPORT_CT_JOYME4)
	add_option125(packet->options, packet->chaddr);
#else
...
static int check_bind(char *wan_if, __u32 mark)
{
	char wan_node[32];
#if defined(TCSUPPORT_MULTI_USER_ITF)
	__u32 pt_mark = GET_LAN_ITF_MARK(mark);
	...
	{
#if defined(TCSUPPORT_MULTI_USER_ITF)
		if(0 != mark_convert_bind_itf_name(pt_mark, lan_if, sizeof(lan_if)))
		{
			return -1;
		}
#else
...
#endif
{
	...
	LOG(LOG_INFO, "sending OFFER of client IP");
#else
...
void update_devicetype_option(uint8_t *vendorID,int len,char *devicetype, char *optionvalue, int devicelen, int optionlen)
{
	char option60[128] = {0};
	if(devicetype == NULL || optionvalue == NULL)
		return;	
	if(vendorID == NULL || len == 0)
	{
		strcpy(devicetype,"PHONE");
		strcpy(optionvalue,"N/A");
		return;
	}
	strncpy(option60,vendorID,len);
	stringToCapital(option60);
	if(strstr(option60,"iptv") != NULL || strstr(option60,"stb"))
	{
		strncpy(devicetype,"STB",devicelen-1);
	}
	else if(strstr(option60,"android") != NULL)
	{
		strncpy(devicetype,"PHONE",devicelen-1);
	}
	else if(strstr(option60,"msft") != NULL)
	{
		strncpy(devicetype,"PC",devicelen-1);
	}
	else if(strstr(option60,"router") != NULL)
	{
		strncpy(devicetype,"AP",devicelen-1);
	}
	else if(strstr(option60,"pad") != NULL)
	{
		strncpy(devicetype,"PAD",devicelen-1);
	}
	else
	{
		strncpy(devicetype,"OTHER",devicelen-1);
	}
	if(strstr(option60,"msft") != NULL && option60[4] == ' ')	
	{		
		snprintf(option60,sizeof(option60)-1,"%s%s","MSFT",option60+5);	
	}
	strncpy(optionvalue,option60,optionlen-1);
	return;
}
#endif
#if defined(TCSUPPORT_CT_FJ)
...
#endif
{
	...
	addr.s_addr = packet.yiaddr;
#if defined (TCSUPPORT_NP_CTC)
	LOG(LOG_INFO, "sending ACK to client");
#else
...
#if defined(TCSUPPORT_CMCCV2)
	length = get_option_len(oldpacket,DHCP_VENDOR);
	vendorID = get_option(oldpacket,DHCP_VENDOR);
	update_devicetype_option(vendorID,length,devicetype,optionvalue,sizeof(devicetype),sizeof(optionvalue));
	add_lease_option(packet.chaddr, packet.yiaddr, lease_time_align, hostname,devicetype,optionvalue);
#else
   ...
   if(0 == tcapi_get("mobile_Entry", "lanenable", lanenable) && !strcmp(lanenable, "1")) {
	...
	snprintf(data, sizeof(data), "add;%02X:%02X:%02X:%02X:%02X:%02X", 
...
int write_ip_vendor_files(uint32_t yiaddr, uint8_t matchid)
{
	...
	while (fread(&tmpaddr, sizeof(uint32_t), 1, fp) == 1)
	{
		...
		res = fread(&tmpmatchid, sizeof(uint8_t), 1, fp);
		if (tmpaddr == yiaddr)
		{       
			flag = 1;
...
void parseDev(struct dhcpMessage *packet, mDev_t *dev)
{
	...
	if(vipos){
		ouilength = (uint8_t)(*(vipos+6));
		snlength = (uint8_t)(*(vipos+8+ouilength));
		pclasslength = *(vipos+10+ouilength+snlength);

		if(ouilength > sizeof(dev->oui))
			ouilength = sizeof(dev->oui);
		
		if(snlength > sizeof(dev->sn))
			snlength = sizeof(dev->sn);
		
		if(pclasslength > sizeof(dev->pclass))
			pclasslength = sizeof(dev->pclass);
...
void writeHostIndexFile(void)
{
	...
	while (tmpnode != NULL)
	{
		...
		sprintf(bufNumber, "HostCount%d=%u\n",i, tmpnode->hostnum);
		fwrite(bufNumber, strlen(bufNumber), 1, fp);
		sprintf(bufIndex, "HostIndex%d=%u %u %u %u %u %u %u %u\n",i, tmpnode->hostindex[0],tmpnode->hostindex[1], \
