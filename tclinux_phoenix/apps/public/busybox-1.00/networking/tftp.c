	...
	)
{
...
#if defined(TCSUPPORT_CT_PMINFORM)
	const int bb_tftp_num_retries = ( retry_time > 0 ) ? retry_time : 
#if defined(TCSUPPORT_CT_MONITORCOLLECTOR)
									3
#else
									5
...
#endif
#if defined(TCSUPPORT_CT_MONITORCOLLECTOR)
	char if_name_tr[32] = {0};
	char isByTr069Wan[8] = {0};
	char startMonitorCollect[8] = {0};
#endif
	...
	memset(&sa, 0, len);
	res = bind(socketfd, (struct sockaddr *)&sa, len);
...
#if defined(TCSUPPORT_CT_PMINFORM)
	if ( ifacename &&
		setsockopt(socketfd, SOL_SOCKET, SO_BINDTODEVICE, ifacename, 32) < 0 )
		tcdbg_printf("\ntftp : setsockopt SO_BINDTODEVICE is faild\n");
#endif
#if defined(TCSUPPORT_CT_MONITORCOLLECTOR)
	if(tcapi_get("MonitorCollector_Common", "startMonitorCollect", startMonitorCollect) == 0 && 
		strcmp(startMonitorCollect,"1") == 0){
		if(tcapi_get("MonitorCollector_Common", "isByTr069Wan", isByTr069Wan) != 0 || strcmp(isByTr069Wan,"1") == 0){
			tcapi_get("CwmpRoute_Entry", "ifName0", if_name_tr);
			tcdbg_printf("tftp :if_name_tr = %s\n",if_name_tr);	
			if(strlen(if_name_tr) != 0){
				if( setsockopt(socketfd, SOL_SOCKET, SO_BINDTODEVICE, if_name_tr, 32) == -1)
					tcdbg_printf("tftp : setsockopt SO_BINDTODEVICE is faild\n"); 	
			}
		}
	}
#endif
...
#define PUT
	...
	while ((opt = getopt(argc, argv, BS GET PUT "l:r:"
#if defined(TCSUPPORT_CT_PMINFORM)
			IFNAME RETRYTIME
#endif
			)) != -1) {
...
#if defined(TCSUPPORT_CT_MONITORCOLLECTOR)
	tcdbg_printf("tftp_main: result = %x\n", result);

	if(tcapi_get("MonitorCollector_Common", "startMonitorCollect", startMonitorCollect) == 0 && 
		strcmp(startMonitorCollect,"1") == 0){
		if(result == EXIT_SUCCESS){
			//remove(remotefile);
			tcapi_set("MonitorCollector_Common","tftpRet","1");
			tcdbg_printf("tftp_main: remotefile = %s\n", remotefile);
		}
	}
#endif
