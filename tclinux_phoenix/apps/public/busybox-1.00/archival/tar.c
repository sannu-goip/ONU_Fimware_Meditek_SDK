								 ...
								 const char *real_name, struct stat *statbuf)
{
	...
	memcpy(header.magic, TAR_MAGIC TAR_VERSION, sizeof(header.magic));
...
# endif
		...
		if (gzipPid == 0) {
			...
			res = fcntl(gzipStatusPipe[1], F_SETFD, FD_CLOEXEC);	/* close on exec shows sucess */
