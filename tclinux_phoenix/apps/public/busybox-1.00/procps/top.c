...
static unsigned long display_generic(void)
{
	 ...
	 * 				-- PFM.
	 */
	if (fscanf(fp, "MemTotal: %lu %79s\n", &total, buf) != 2) {
		fgets(buf, sizeof(buf), fp);	/* skip first line */

		res = fscanf(fp, "Mem: %lu %lu %lu %lu %lu %lu",
		...
		res = fscanf(fp, "MemFree: %lu %79s\n", &mfree, buf);

		/*
		 * MemShared: is no longer present in 2.6. Report this as 0,
		 * to maintain consistent behavior with normal procps.
		 */
		if (fscanf(fp, "MemShared: %lu %79s\n", &shared, buf) != 2)
			shared = 0;

		res = fscanf(fp, "Buffers: %lu %79s\n", &buffers, buf);
		res = fscanf(fp, "Cached: %lu %79s\n", &cached, buf);
