		...
		for (sed_cmd = sed_cmd_head.next; sed_cmd; sed_cmd = sed_cmd->next) {
			...
			if(matched) {
				sed_cmd->in_match = !(
					...
					(pattern_space && regexec(sed_cmd->end_match, pattern_space, 0, NULL, 0) == 0))
				);
			}
...
discard_commands:
		...
		if (!be_quiet && pattern_space) sed_puts(pattern_space,no_newline);
