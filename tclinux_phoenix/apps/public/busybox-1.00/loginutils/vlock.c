...
static void release_vt(int signo)
{
	int res = 0;
	if (!o_lock_all)
		res = ioctl(vfd, VT_RELDISP, 1);
	else
		res = ioctl(vfd, VT_RELDISP, 0);
}

static void acquire_vt(int signo)
{
	int res = 0;
	res = ioctl(vfd, VT_RELDISP, VT_ACKACQ);
}

static void restore_terminal(void)
{
	int res = 0;
	res = ioctl(vfd, VT_SETMODE, &ovtm);
...
extern int vlock_main(int argc, char **argv)
{
	...
	do {
		...
		if (strncmp(crypt_pass, pw->pw_passwd, strlen(crypt_pass)) == 0) {
