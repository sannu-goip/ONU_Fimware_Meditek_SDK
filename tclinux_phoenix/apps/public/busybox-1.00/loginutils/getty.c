...
int getty_main(int argc, char **argv)
{
	...
	if (!(options.flags & F_LOCAL)) {
		...
		res = fcntl(1, F_SETFL, fcntl(1, F_GETFL, 0) & ~O_NONBLOCK);
	}
	...
	(void) execl(options.login, options.login, "-c", logname, (char *) 0);
#else
...
static void update_utmp(char *line)
{
	...
	if (utp) {
		...
		strncpy(ut.ut_id, line + 3, sizeof(ut.ut_id) - 1);
	}
	/*endutent(); */

	strncpy(ut.ut_user, "LOGIN", sizeof(ut.ut_user) - 1);
	strncpy(ut.ut_line, line, sizeof(ut.ut_line) - 1);
	if (fakehost)
		strncpy(ut.ut_host, fakehost, sizeof(ut.ut_host) - 1);
	time(&t);
	ut.ut_time = t;
...
static void termio_init(struct termio *tp, int speed, struct options *op)
{
	...
	res = fcntl(0, F_SETFL, fcntl(0, F_GETFL, 0) & ~O_NONBLOCK);
