...
arp [B<-evn>] [B<-H> type] [B<-i> if] B<-a> [hostname]
	arp [B<-v>] [B<-i> if] B<-d> hostname [pub]
	arp [B<-v>] [B<-H> type] [B<-i> if] B<-s> hostname hw_addr [temp]
	arp [B<-v>] [B<-H> type] [B<-i> if] B<-s> hostname hw_addr [netmask nm] pub
	arp [B<-v>] [B<-H> type] [B<-i> if] B<-Ds> hostname ifa [netmask nm] pub
	arp [B<-vnD>] [B<-H> type] [B<-i> if] B<-f> [filename]
...
ip [ OPTIONS ] { address | link | route | tunnel | neigh} { COMMAND | help }
...
=item B<udhcpc_usbDongle>

udhcpc_usbDongle [B<-fbnqv>] [B<-c> CLIENTID] [B<-H> HOSTNAME] [B<-i> INTERFACE]
[B<-p> pidfile] [B<-r> IP] [B<-s> script]
