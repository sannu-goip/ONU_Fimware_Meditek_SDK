...
#if defined(TCSUPPORT_CMCCV2) || defined(TCSUPPORT_CUC) || defined(TCSUPPORT_CT_JOYME4) || defined(TCSUPPORT_NP_CTC)
#define MAX_LOGFILE_SIZE (1024*60-15) /* 60k - strlen(EndOfTCSysLog\r\n) */
#else
...
#if defined(TCSUPPORT_CT_PON_SC) || defined(TCSUPPORT_CMCCV2) || defined(TCSUPPORT_CUC) || defined(TCSUPPORT_CT_JOYME4) || defined(TCSUPPORT_NP_CTC)
...
int check_CurrLog(void)
{
	FILE *fp = NULL;
	char deviceInfoBuf[512];
	char buf[MAXLINE] = {0};
	char LogHead[20] = "Manufacturer";
	
	fp = fopen(tmpLogFilePath, "r+");
	if(fp == NULL) {
		printf("\n==>Syslog Error, read logMsg, read open file %s failed \n", tmpLogFilePath);
		return -1;
	}
	while(fgets(buf, MAXLINE, fp)) {
		char *s = strstr(buf, LogHead);
		if( s != NULL ) 
		{
			fclose(fp);
			return 0;
...
int read_logMsg_from_flash(void)
{
	...
	res = fseek(curr_fp, strlen(deviceInfoBuf), SEEK_SET);
#if defined(TCSUPPORT_CMCCV2)
	}
	else
	{
		res = fseek(curr_fp, 0, SEEK_SET);
	}
#endif
#endif
...
#if !defined(TCSUPPORT_CMCCV2) && !defined(TCSUPPORT_CUC) && !defined(TCSUPPORT_CT_JOYME4)
	...
	if( delLogNum <= 0 ) {/* If currLogFile is no full */
		...
		if(currLogFileSize > MAX_LOGFILE_SIZE) {
			...
			while(1) {
				...
				if(currLogFileSize <= MAX_LOGFILE_SIZE) {
					res = rename(tmpLogFilePath, currLogFilePath);
