	...
	if ((pid = fork()) == 0) {
		...
		if (a->action & ASKFIRST) {
			...
			bb_full_write(1, press_enter, sizeof(press_enter) - 1);
                        ...
                        int i = 0;
			char status[5] = {0};
			while(!tcapi_get("XPON_Common", "trafficStatus", status) && ++i < 100)
			{
				if(!strcmp(status,"up"))
				{
					sleep(3);
...
static void shutdown_system(void)
{
	sigset_t block_signals;
   	...
   	while(1)
	{  
        	if(tcapi_get("Info_Oam","SaveFlag",SaveFlag )!=0 || \
			   0==atoi(SaveFlag)  						     || \
               count>10 )
			{
            		break;  /*if SaveFlag is not created or is 0(which means tcapi_save complete)*/
            }      		/*when some problem cause save opration takes long time,we must let*/
        	else
			{           /*shuntdown go on,so we add a count here*/
            		count++;
            		sleep(1);
		    }
	}
	...
	message(CONSOLE | LOG, "Disable PON PHY!!");
...
static void new_init_action(int action, const char *command, const char *cons)
{
	...
	strncpy(new_action->terminal, cons,sizeof(new_action->terminal)-1);
...
extern int init_main(int argc, char **argv)
{
	...
	printf("busybox init and set aff=8=\n");
	unsigned i = 0;
	//only use TC1 to application. shnwind 20110318.
		
	blapi_system_get_cpu_default_affinity(&aff);
