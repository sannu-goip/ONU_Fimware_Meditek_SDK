...
void print_content_type(request * req)
{
	...
	if( strstr(req->request_uri, ".gif") /* for FIX HP SCAN 11039 */
		|| strstr(req->request_uri, ".jpg") )
	{
		p_extension = strrchr(req->request_uri, '/');
		if ( p_extension )
		{
			snprintf(dispo, sizeof(dispo)
...
#if defined(TCSUPPORT_CT_JOYME2)
	/* for FIX HP SCAN 11293 Cross-Frame Scripting. */
...
{
#if defined(TCSUPPORT_CT_JOYME2)
#if defined(TCSUPPORT_CT_DBUS) && !defined(TCSUPPORT_CUC)
	send_r_moved_temp(req, "/cgi-bin/redirect.asp", "");
#else
	send_r_moved_temp(req, "/cgi-bin/login.asp", "");
#endif
