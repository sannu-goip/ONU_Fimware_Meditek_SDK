...
#define CA_CERT_FILE "/usr/etc/boa_ca.crt"
#define SERVER_CERT_FILE "/usr/etc/boa_server.crt"
#define SERVER_KEY_FILE "/usr/etc/boa_server.key"
#define SSL_FILETYPE_PEM 1

#ifdef INET6
struct sockaddr_in6 openssl_server_sockaddr;	/* boa ssl socket address */
#else
struct sockaddr_in openssl_server_sockaddr;		/* boa ssl socket address */
#endif
int server_openssl;					/*ssl socket */
int openssl_server_port = 443;		/*The port that the server should listen on*/
int openssl_pending_requests = 0; 
static int sock_opt = 1;
SSL_CTX *sslCTX = NULL;
...
{
	tcdbg_printf( "\nEnabling SSL security system");
	if ( (server_openssl = socket(SERVER_AF, SOCK_STREAM, IPPROTO_TCP)) == -1 )
	{
		tcdbg_printf("\nCouldn't create socket for ssl");
		DIE("Couldn't create socket for ssl");
	}

	if ( setsockopt(server_openssl, SOL_SOCKET, SO_REUSEADDR, (void *) &sock_opt, sizeof(sock_opt)) == -1 )
	{
		tcdbg_printf("\nCouldn't sockopt"); 
		DIE("Couldn't sockopt");
	}
	/* internet socket */
#ifdef INET6
	openssl_server_sockaddr.sin6_family = AF_INET6;
	memcpy(&openssl_server_sockaddr.sin6_addr, &in6addr_any, sizeof(in6addr_any));
	openssl_server_sockaddr.sin6_port = htons(openssl_server_port);
#else
	openssl_server_sockaddr.sin_family = AF_INET;
	openssl_server_sockaddr.sin_addr.s_addr = htonl(INADDR_ANY);
	openssl_server_sockaddr.sin_port = htons(openssl_server_port);
#endif

	if ( bind(server_openssl, (struct sockaddr *) &openssl_server_sockaddr,sizeof(openssl_server_sockaddr)) == -1 )
	{
		tcdbg_printf("\nCouldn't bind ssl to port"); 
		DIE("Couldn't bind ssl to port");
	}
	...
	/* listen: large number just in case your kernel is nicely tweaked */
	if ( listen(server_openssl, backlog) == -1 )
	{
		tcdbg_printf("\nCouldn't listen"); 	
		DIE("Couldn't listen");
	}

	if ( boa_openSslReadKeys(&sslCTX) == -1 )
	{
		tcdbg_printf("\nCouldn't read SSL keys!");
		DIE("\nCouldn't read SSL key!");
	}
...
int boa_openSslReadKeys(SSL_CTX **ctx)
{
	...
	if ( NULL == meth )
	{
		tcdbg_printf("\nboa_openSslReadKeys: meth is NULL!");
		goto SSL_CTX_ERR;
	}
	...
	*ctx = SSL_CTX_new (meth);
	if ( NULL == ctx )
	{
		tcdbg_printf("\nboa_openSslReadKeys: ctx is NULL!");
		goto SSL_CTX_ERR;
	}
	...
	{
		tcdbg_printf("\nboa_openSslReadKeys: SSL_CTX_load_verify_locations error!");
	...
	{
		tcdbg_printf("\nboa_openSslReadKeys: SSL_CTX_use_certificate_file error!");
		ERR_print_errors_fp(stderr);
		goto SSL_CTX_ERR;
	}
	...
	{
		tcdbg_printf("\nboa_openSslReadKeys: SSL_CTX_use_PrivateKey_file error!");
		ERR_print_errors_fp(stderr);
		goto SSL_CTX_ERR;
	}
	...
	{
		tcdbg_printf("\nboa_openSslReadKeys: SSL_CTX_check_private_key error!");
		ERR_print_errors_fp(stderr);
		goto SSL_CTX_ERR;
	}
