...
#if 0
#if defined(TCSUPPORT_CT_E8GUI)
		real_numargs = numargs;
#endif
#endif

	    while (*cp) {
/* reserve the solution: duplicated html control submit may cause process over range. */
#if 0
#if defined(TCSUPPORT_CT_E8GUI)
			/* it over range items. */
			if ( i >= real_numargs )
...
#if 0
#if defined(TCSUPPORT_CT_E8GUI)
		/* should reduce total numer when duplicated name occurs */
		real_numargs --;
#endif
#endif
	    }
...
int multipart_parser(request *req, char *boundary){
	...
	int fd = -1, len = 0, bound_len = 0, read_size = 0, res = 0;
	int pos = 0;
	int pos_offset = 0;
	...
	while(pos < len){
		...
		res = read(req->post_data_fd,temp,read_size);
...
find_data_end(int bound_pos ,int fd, int write_fd){
	...
	*/
	...
	if(bound_pos >= MULTI_PASER_TEMP_SIZE){
		res = lseek(fd, (bound_pos-MULTI_PASER_TEMP_SIZE), SEEK_SET);
		res = read(fd,buf,MULTI_PASER_TEMP_SIZE);
		check_start = MULTI_PASER_TEMP_SIZE;
		
	}else{
		res = lseek(fd, 0, SEEK_SET);
		res = read(fd,buf,bound_pos);
