...
{
    int data_fd = -1, saved_errno = -1, res = -1;
    struct stat statbuf;
    volatile int bytes;
    ...
    res = fstat(data_fd, &statbuf);

    if (S_ISDIR(statbuf.st_mode)) { /* directory */
        close(data_fd);         /* close dir */

        if (req->pathname[strlen(req->pathname) - 1] != '/') {
            static char buffer[3 * MAX_PATH_LENGTH + 128];
    ...
    if (req->if_modified_since &&
		...
		&& ( 0 != strcmp("/index.html", req->request_uri) && 0 != strcmp("/cu.html", req->request_uri) )
#endif
#if defined(TCSUPPORT_CT_PON_JX)
		&& ( 0 != strcmp("/index.html", req->request_uri) && 0 != strcmp("/admin.html", req->request_uri) )
#endif
#if defined(TCSUPPORT_CGNX)
		&& ( strstr(req->request_uri, "captcha_") == NULL)
#endif
		) {
...
int get_dir(request * req, struct stat *statbuf)
{
    ...
    if (directory_index) {      /* look for index.html first?? */
        strncpy(pathname_with_index, req->pathname, sizeof(pathname_with_index) - 1);
        strncat(pathname_with_index, directory_index, sizeof(pathname_with_index) - strlen(pathname_with_index) - 1);
        /*
        ...
        if (data_fd != -1) {    /* user's index file */
            strncpy(req->request_uri, directory_index, sizeof(req->request_uri) - 1); /* for mimetype */
            res = fstat(data_fd, statbuf);
            return data_fd;
        }
        if (errno == EACCES) {
